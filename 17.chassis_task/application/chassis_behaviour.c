...
static void chassis_no_follow_yaw_control_gyop(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector);
...
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    if (vx_set == NULL || vy_set == NULL || wz_set == NULL || chassis_move_rc_to_vector == NULL)
    {
		...
		//给一个定的旋转速度
		if(switch_is_mid(chassis_move_rc_to_vector->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL+1]))
		{	

				*wz_set=CHASSIS_AUTOGYRATION_VALUE;	
		}			
		else if(switch_is_down(chassis_move_rc_to_vector->chassis_RC->rc.s[CHASSIS_MODE_CHANNEL+1]))
		{
